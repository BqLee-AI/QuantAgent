...
- 大文件问题不只按行数判断，而是看是否有多个变化原因。一个文件应只有一个主要修改理由；如果为了减少文件数量而牺牲职责清晰，应优先职责清晰。
- 关键意图必须注释：安全边界、审计、状态机、权限、并发、幂等、重试、协议适配、生成物边界和非显然取舍必须用短注释说明“为什么这样做”。
- 设计模式按需使用。有多实现、稳定接口、外部适配、可替换策略、生命周期、持久化或插件隔离边界时，优先考虑 service、repository、port、adapter、strategy、registry、base class 等结构；没有真实复用和稳定职责时，不要为了模式本身提前抽象。
- OpenSpec、issue 和 PR 都必须能证明工程质量门槛已经被考虑；如果缺少架构分层、复用判断、失败路径或验证口径，AI 应先补文档或追问，而不是继续实现。
...
- AI 生成的文档主体内容应使用中文；代码标识、命令、路径、协议字段和专有名词可保留原文。
