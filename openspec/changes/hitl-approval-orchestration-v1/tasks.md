...
- [x] 2.1 后续实现同步 `DEFAULT_EVENT_TOPICS`，加入 `action.requested` 和 `approval.input_received`。
- [x] 2.2 后续实现更新 `packages/core/src/quantagent/core/events/README.md` 的默认 topic 和新增 topic 说明。
- [x] 2.3 后续实现补充 Event Bus contract tests，验证新增 topic 被接受、未知 topic 仍被拒绝。
- [x] 2.4 后续实现核对 envelope payload 只包含 JSON-safe 数据，不包含 ORM、DB session、API envelope、插件实例或不可序列化对象。

## 3. Core Approval 模型与策略
...
## 4. Repository、Ports 与 Publishers
...
## 5. Orchestration 与 Event Handlers
