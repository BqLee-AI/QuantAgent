...
- `harness.py`：提供测试专用 fake AI producer、fake notification consumer、human authorization message builder、fake human input producer 和 fake executor。
...
### 6. 通知链路使用 fake consumer 和脱敏消息 builder

`notification.requested` 被测试专用 fake notification consumer 消费，生成 `HumanAuthorizationMessage`：
...
真实 Discord / Telegram / Email / WebSocket provider 和投递状态不进入本轮。
...
fake AI / Decision producer
  ...
  -> fake notification consumer
  -> HumanAuthorizationMessage
  -> fake human input producer
  -> EventEnvelope(topic=approval.input_received)
...
- [Risk] fake notification consumer 不能证明真实通知适配正确。
  Mitigation：本轮只验证消息形状和脱敏边界，真实 provider 由后续 notification issue 承接。
