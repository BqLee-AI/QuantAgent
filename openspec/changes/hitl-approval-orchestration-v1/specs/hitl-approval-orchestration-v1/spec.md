...
系统 SHALL 将 `notification.requested` 转换为脱敏的 `HumanAuthorizationMessage`，用于测试人工授权消息生成边界。
...
系统 SHALL 通过 `approval.input_received` 接收人工输入，并将输入保存、评估和转换为审批决策。
...
系统 SHALL 提供测试专用 harness，使用 InMemoryEventBus 覆盖从行为请求到审批完成的核心链路。
...
- **AND** fake human input producer 发布 `approval.input_received`
