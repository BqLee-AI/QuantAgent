...
本 change 先用 OpenSpec 固定 `action.requested -> approval.requested / notification.requested -> approval.input_received -> approval.completed` 的 core-only 消息闭环，让后续实现 PR 有同一条证据链。
...
- 固定第一刀范围：使用 `InMemoryEventBus` 与 fake producer / consumer / executor / PolicyGate 跑通审批消息闭环，不接 API、数据库、Web、真实通知插件或真实 broker。
