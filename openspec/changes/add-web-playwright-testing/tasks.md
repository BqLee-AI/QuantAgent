...
- [ ] B2. 为 `apps/web` 接入 Playwright 基础配置与标准命令
...
- [ ] B2.5. 验证 Playwright `webServer.command` 与当前 Vite 启动方式兼容
  - Input: `apps/web` 当前 `dev` script 为 `vite`、固定端口 `5173`、Playwright `webServer` 配置方案
  - Output: 确认 `bun --cwd apps/web run dev -- --host 127.0.0.1 --port 5173` 是否可用；若不可用，则切换到 `bunx vite --host 127.0.0.1 --port 5173` 或等效 Bun 方案
  - Output: 确认 Playwright 能等待本地 server 就绪，并与 `reuseExistingServer: true` 协同工作
  - Write boundary: `apps/web/playwright*.config.*`, `openspec/changes/add-web-playwright-testing/**`

- [ ] B3. 建立 Chromium smoke test 与报告/诊断策略
...
- [ ] B4. 启用 Playwright Component Testing 基础承载
...
- [ ] R1. 审阅 spec 中关于 Chromium-only、CI 延后、Component Testing 启用与测试边界的约束后，再进入实现。
- [ ] R2. Playwright 配置与目录约定落地后，确认其不会与现有 Vitest unit test glob 冲突。

## Verification

- [ ] `bun --cwd apps/web run test:e2e` 通过
- [ ] `bun --cwd apps/web run test:e2e:ui` 可启动 Playwright UI 模式
- [ ] Chromium 浏览器二进制已通过 `bunx playwright install chromium --with-deps` 或等效 Bun 命令安装可用
- [ ] `bun run lint` 通过
- [ ] `bun run build --filter=web` 通过
- [ ] `openspec validate add-web-playwright-testing --type change --strict --json` 通过
