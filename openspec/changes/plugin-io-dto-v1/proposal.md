...
- 定义 Plugin IO DTO V1 的分层：保留 Runtime V1 通用 `PluginInvokeRequest` / `PluginInvokeResult` 作为 transport，在其上补 capability-specific typed DTO。
- 定义 DTO 真源形态：沿用当前 `packages/plugin-sdk` 的冻结 `dataclass` 风格，而不是切换到另一套模型系统。
...
- 定义 DTO 的序列化、可审计和脱敏要求，并保持结构化错误继续沿用 Runtime V1 已有 `PluginError` 形状。
...
- `packages/plugin-sdk/**`：后续实现 typed DTO dataclass、必要的序列化 helper 和 capability-specific contract test。
