...
### Requirement: DTO 真源沿用冻结 dataclass 风格

QuantAgent SHALL keep Plugin IO DTO V1 aligned with the existing plugin-sdk frozen dataclass style.

#### Scenario: typed DTO 与 Runtime DTO 风格一致
- **WHEN** plugin-sdk 暴露 `source.fetch` 或 `notification.send` typed DTO
- **THEN** DTO 使用冻结 dataclass 或等价只读对象风格
...
- **AND** 输入字段适合表达 source fetch 所需的最小公共信息和 metadata
