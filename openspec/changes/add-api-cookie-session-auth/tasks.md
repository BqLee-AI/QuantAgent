...
当前状态为 OpenSpec artifact 准备阶段。实现必须等待 OpenSpec-only PR 获得维护者明确认可。

## 任务图

### 阻塞串行路径

- [ ] B0. OpenSpec-only artifacts 审核
...
- [ ] B2. 实现 session 与 CurrentActor 基础
...
- [ ] B4. 扩展 401/403 AppError envelope
...
- [ ] B7. 实现 actor/audit context helper
...
- [ ] R1. 确认本 change 没有引入用户注册、RBAC、多租户、OAuth、SSO、用户表、session 表或 audit persistence。
- [ ] R2. 确认 production 默认不会关闭鉴权，不会默认 insecure cookie；确认当前 local Docker compose 未被文档或实现误判为 production 安全默认。
- [ ] R3. 确认 public route 只包含系统探针和非业务 version route；业务 API 默认 protected。
- [ ] R4. 确认 router 不直接手写 cookie 解析、capability 字符串判断或 CSRF 校验。
- [ ] R5. 确认 `/me`、错误响应、日志和测试输出不泄露 session、cookie、secret、口令、hash、私有策略或 stack trace。
- [ ] R6. 确认 actor/audit context 只传递脱敏 actor 与 request metadata，供后续高风险 API 复用。
- [ ] R7. 确认 OpenAPI routes 使用 `ApiResponse[T]` envelope、显式 tags，并且 production OpenAPI 不暴露 debug-only paths。

## 验证任务

- [ ] V1. 验证 OpenSpec change：`openspec validate add-api-cookie-session-auth --type change --strict --json`
- [ ] V2. 实现阶段运行 API 测试：`cd apps/api && uv run python -m unittest discover -s src/tests`
