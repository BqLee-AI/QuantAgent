...
实现阶段阻塞中，必须先完成本 OpenSpec change 的人工审核并获得明确批准。

## 任务图

### 阻塞串行路径

- [ ] B0. 实现前审核 OpenSpec artifacts
...
- [ ] B3. 将现有 health/ready 探针 route 纳入同一显式契约
...
- [ ] P1. 增加 API route 与 OpenAPI 契约测试
...
- [ ] R1. 确认 sample resource 命名和 payload 不暗示 runtime、metadata、plugin、approval、Agent、tool invocation、WebSocket、executor 或 live trading 能力。
- [ ] R2. 确认 provider/sample data 只表达替换点，不包含数据库访问、核心领域逻辑、runtime health state、registry state、credentials 或外部服务调用。
- [ ] R3. 确认 production app 和 production OpenAPI schema 都排除 debug-only routes。
- [ ] R4. 确认 README 不声称 API resources、OpenAPI artifacts、generated clients 或业务 endpoint families 已完成。
- [ ] R5. 确认 README 的最小验证命令能作为后续新增 API route 的默认本地验证入口。
- [ ] R6. 确认 router 注册 helper、显式 `response_model` 和测试调整没有破坏 PR #70 已有 `/api/v1/ready` readiness 行为、503 envelope、敏感信息不泄露和 lifespan 资源清理。

## 验证任务

- [ ] V1. 运行 API 测试：`cd apps/api && uv run python -m unittest discover -s src/tests`
- [ ] V2. 验证 OpenSpec change：`openspec validate api-v1-route-contract-skeleton --type change --strict --json`
