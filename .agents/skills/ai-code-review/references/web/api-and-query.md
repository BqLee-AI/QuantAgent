...
- `shared/api/base-api.ts` 只负责 `basePath`、endpoint path 拼接和 `get/post/put/patch/del/requestEnvelope` 委托；多 `baseURL` 用多个 client 实例，多 base path 用 `baseApi.withBasePath(...)`。
- 业务 endpoint 默认进入 `features/<domain>/api.ts`，服务端状态进入 `features/<domain>/queries.ts` / `mutations.ts`。
- feature `api.ts` 只封装 endpoint 和 DTO，不放 React state、TanStack Query cache、view state 或 UI。
- Auth 是横切能力，可以放在 `shared/auth/api.ts`，但仍必须走 `createAuthApi(baseApi)`，不能复制旧 `context.tsx` 包办请求、状态和 React 生命周期的模式。
...
6. 检查文件职责是否单一：client 配置、dedupe、envelope、errors、base path、feature endpoint、query hook 和 UI 是否分开。
...
export function createPluginsApi(baseApi: BaseApi) {
  const pluginsApi = baseApi.withBasePath("/plugins");

  return {
    list(params: PluginListParams) {
      return pluginsApi.get<PluginListResponse>("/", { params });
    },
  };
}
...
export function usePluginList(params: PluginListParams) {
  ...
  const pluginsApi = useMemo(() => createPluginsApi(baseApi), [baseApi]);
...
```ts
// shared/auth/api.ts
export function createAuthApi(baseApi: BaseApi) {
