...
涉及 Web 的 issue 不能只写“做页面”或“接 API”。必须根据 `.agents/skills/references/web-architecture-gate.md` 收住 route、runtime/API、query/mutation、业务 hook、view component、README、中文注释和目录分组边界，并写进 `子任务树` 或 `架构 / 分层风险`。涉及复杂 feature 或文件拆分时，还必须对照 `.agents/skills/references/web-file-responsibility-and-feature-structure.md` 写清目标目录和文件职责。
